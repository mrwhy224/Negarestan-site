...
<div class="flex justify-between items-center mb-6">
    <div>
        <h1 class="text-3xl font-bold text-[var(--brand-blue)]">{{ isset($question) ? 'ویرایش سوال' : 'افزودن سوال جدید' }}</h1>
        <nav class="text-sm font-semibold mt-2" aria-label="Breadcrumb">
            <ol class="list-none p-0 inline-flex">
                ...
                <li class="mx-2 text-gray-400">/</li>
                <li class="flex items-center">
                    <a href="{{ route('quiz.list') }}" class="text-gray-500 hover:text-gray-700">سوالات</a>
                </li>
                <li class="mx-2 text-gray-400">/</li>
                <li class="text-gray-800">{{ isset($question) ? 'ویرایش' : 'افزودن' }}</li>
            </ol>
        </nav>
    </div>
</div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6 lg:p-8">
        ...
        <form id="question-form" action="{{ isset($question) ? route('quiz.updateQuestion', ['quiz' => $quiz->id, 'question' => $question->id]) : route('quiz.addQuestion', ['quiz' => $quiz->id]) }}" method="POST">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea id="question_text"
                              name="question_text"
                              rows="4"
                              class="w-full px-4 py-2 border rounded-lg bg-gray-50 focus:outline-none focus:ring-2 focus:ring-yellow-400 @error('question_text') border-red-500 @enderror"
                              placeholder="سوال را اینجا وارد کنید."
                              required>{{ old('question_text', $question->text ?? '') }}</textarea>
                    @error('question_text')
                ...
                <div class="flex items-center space-x-4 space-x-reverse">
                    <div class="flex-grow">
                        ...
                        <select id="question_type"
                                name="question_type"
                                class="w-full px-4 py-2 border rounded-lg bg-gray-50 focus:outline-none focus:ring-2 focus:ring-yellow-400 @error('question_type') border-red-500 @enderror"
                                required>
                            <option value="single" {{ old('question_type', $question->type ?? '') == 'single' ? 'selected' : '' }}>تک پاسخی</option>
                            <option value="multiple" {{ old('question_type', $question->type ?? '') == 'multiple' ? 'selected' : '' }}>چند پاسخی</option>
                        </select>
                        @error('question_type')
                            <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                        @enderror
                    ...
                    @if(isset($question) && $question->options->count() > 0)
                        @foreach($question->options as $index => $option)
                            <div class="option-row flex items-center space-x-2 space-x-reverse">
                                <input type="text" name="options[{{ $index }}][text]" value="{{ $option->text }}" class="w-full px-4 py-2 border rounded-lg bg-gray-50" placeholder="متن گزینه" required>
                        ...
                        @endforeach
                    @else
                        {{-- گزینه‌های اولیه برای سوال جدید --}}
                        <div class="option-row flex items-center space-x-2 space-x-reverse">
            ...
            <div class="flex justify-end items-center mt-8 pt-6 border-t border-gray-200">
                ...
                <button type="submit" class="bg-[var(--brand-gold)] text-white font-bold py-2 px-6 rounded-lg hover:bg-yellow-500 transition-colors shadow-md">
                    {{ isset($question) ? 'به‌روزرسانی' : 'ذخیره' }}
                </button>
            </div>
        </form>
    </div>
